...
def predictions_visuals(y_test, y_pred, model_name="Model", n_samples=2000):
    ...
    # RANDOM SAMPLING — THIS IS THE KEY
    idx = np.random.choice(len(y_test), size=min(n_samples, len(y_test)), replace=False)
    idx.sort()  # keeps chronological order for time series plot
    
    y_t = y_test.iloc[idx].values
    y_p = y_pred[idx]
    
    # Left: scatter
    axes[0].scatter(y_t, y_p, alpha=0.6, s=15, color='#2c7bb6')
